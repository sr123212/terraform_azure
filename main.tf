...
resource "azurerm_network_interface" "my_terraform_nic2" {
  ...
  ip_configuration {
    name                          = "my_nic_configuration2"
    subnet_id                     = azurerm_subnet.my_terraform_subnet.id
    private_ip_address_allocation = "Dynamic"
  }
}

# Connect the security group to the network interface_1
...
# Connect the security group to the network interface_2
